...
- `~/CrossDesk-materialy/` (outside the repo) — capture material kept
  for reference, chiefly `pararelInstaltionProcessVideo.mov` (702 MB,
  sha256 `f1b60a3f…1c4e2f`), transferred off the MacBook 2026-07-22.
  Binaries this size never enter git — `.gitignore:78` excludes `*.mov`
  — so the path is recorded here instead of being silently untracked.
