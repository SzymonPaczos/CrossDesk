...
| 3 | plik -> app Windows przez skonfigurowany share | 🔲 box | **DEC-0019 (podpisane 2026-07-19):** MVP-floor = Stage B, **default scope `documents`** + **JIT-lite** (per-launch share katalogu pliku, gaśnie z sesją); whole-`$HOME` = opt-in za ostrzeżeniem. Host-side gotowe (default+ostrzeżenie+JIT-lite w kodzie); live-verify mountu Stage B + Save dialog 🔲 box. **2026-08-24:** dwa gate'y bezpieczeństwa przed live-verify **padły** — SEC-02 (JIT-lite dzielił całe `$HOME` dla pliku w korzeniu, `1243070`) i Red-Team B (`ShareChannel` token-authz + escapowany virtiofs XML, `1adf106`). Realny bloker to nadal **guest-side provisioning** (WinFsp+viofs+VirtioFsSvc, pozycja w NEXT) |
...
| 5 | suspend/resume bez false HARD_DESTROY | 🔲 owner | Mechanizm gotowy i **sentinel-testowany** (A5 `c4cb6e8`: freeze-test + „FSM-still-live” test). Ale realna akceptacja = **prawdziwy sleep hosta**, a tego **nie da się wykonać autonomicznie**: listener słucha `login1.PrepareForSleep` na **system-busie** (nie do podrobienia bez roota), `rtcwake` wymaga roota (`sudo -n` → prompt), a każdy realny suspend zawiesza sam box operatora i jego sesję. To jest 5-minutowy eyeball właściciela — runbook w `needs-owner.md`. Zaparkowane 2026-07-25 (loop C5). |
...
**Aktualizacja 2026-08-24 (pętla remediacyjna audytu 2026-08-23).** Tabela **bez
zmian** — remediacja nie dotykała kryteriów, tylko długu, który je poprzedza.
Wyczerpana kolejka R1–R4: SEC-02 (`1243070`), Red-Team B (`1adf106`), F-2
(`e5d672b`), P0 gate `pre-push` A1 (`a6efb19`); po drodze dwa blokery bramek —
RUSTSEC-2026-0258/h2 (`347c30c`) i czerwony main po clippy 1.98 (`ec427a9`).
Cztery żywe pozycje (#3, #5, #11, #12) pozostają zaparkowane na **gościu /
sprzęcie / eyeballu / tagu**, nie na kodzie hosta.
