...
- [2026-05-10 14:30] START · agent: claude-code · branch: chore/threat-model-credential-drift · task: threat-model-credential-drift · note: User-approved (option a) THREAT_MODEL.md update for credential-drift attack vector after Stage 1+2+3 verify-credentials feature loop. Add row to C7 (Configuration store) STRIDE table with marker "🚧 mock coverage; real LogonUserW pending Stage 4". Doc-only, no code. Branch left for user merge.
- [2026-05-10 14:50] START · agent: claude-code · branch: feat/code-signing · task: code-signing-self-trusted · note: User-approved (I+II) — self-signed publisher PKI for agent.exe + autounattend installs root CA into guest Cert:\LocalMachine\Root + Defender exclusion for C:\CrossDesk\. Stage A: PKI generation script (root CA + leaf signing cert + PFX bundle). Stage B: sign-agent.sh wrapping osslsigncode. Stage C: autounattend.xml — certutil -addstore Root + Add-MpPreference exclusions. Stage D: docs/CODE_SIGNING.md + infra/code-signing/README.md + .gitignore for private keys. Stage E: tests where possible on Mac (PKI valid + verify roundtrip). Zero annual cost; only works because agent.exe never leaves our orchestrated guest VM. CI release-pipeline integration deferred (release.yml not yet wired in this repo).
