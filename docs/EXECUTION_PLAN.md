...
- ✅ **[P0] In-process integration test harness.** `host/tests/test_smoke_inprocess.py` boots the Python host gRPC server on a free port, spawns the Rust agent built with `--features mock` (which skips the Windows SCM dispatcher and runs the planes under tokio), and asserts the full mTLS + AuthContext + ClientHello → ServerAccept handshake completes end-to-end. Surfaced and fixed three pre-existing bugs in the same commit (wrong cert fingerprint stamped, `http://` URL bypassing TLS, private grpc-python API in filesystem.py). Installer/Launch/RailWindowEvent flow stays a stretch goal for Phase 4 — the harness exercises whatever surface IS implemented today.
