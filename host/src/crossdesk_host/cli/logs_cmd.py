...
def _parse_iso(ts: str) -> Optional[datetime]:
    ...
    # Strip sub-second fractions that exceed 6 digits (also Python 3.9 limit).
...
async def _tail_file(path: Path, source: str) -> AsyncIterator[_LogRecord]:
    ...
    Seeks to the end on first open, then polls for new content using
    ``asyncio.sleep`` with a short interval.  This avoids the inotify /
    kqueue dependency while keeping the loop non-blocking.
...
async def _follow_sources(
    ...
    queue; a consumer task drains the queue and prints.  Tasks are cancelled
    ...
    for src in sources:
        if src == "host":
            ...
            journal_iter = _tail_journalctl(src)
            tasks.append(asyncio.create_task(_pump(journal_iter)))
