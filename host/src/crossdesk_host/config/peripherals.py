...
class PeripheralsConfig(BaseModel):
    ...
    @classmethod
    def _printer_name_required_when_named(cls, v: str, info: object) -> str:
        # Pydantic v2 passes a FieldValidationInfo; we only need the value
        # here.  The cross-field check is intentionally deferred to
        # model_post_init so both fields are available.
        return v
    ...
    def model_post_init(self, __context: object) -> None:
        # Cross-field validation: printer_name must be non-empty when mode is "named".
        # Cannot use @field_validator for this because both fields must be resolved.
